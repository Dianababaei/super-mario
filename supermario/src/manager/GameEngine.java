...
public class GameEngine implements Runnable {
    ...
    private void init() {
        ...
        public void windowClosing(java.awt.event.WindowEvent ae) {
            GameState game = new GameState( getRemainingLives(), getRemainingTime(), getCoins(), getScore());
            if (user.getHighestScore() < game.totalScore(getRemainingLives(), getCoins(), getRemainingTime(), getScore())){
                user.setHighestScore(game.totalScore(getRemainingLives(), getCoins(), getRemainingTime(), getScore()));
            }
            //TODO fix this line this is important
            user.getRunningGames().add(0,game);
            FileWriter file = null;
            try {
                file = new FileWriter("./src/"+ user.getName()+".json");
                new Gson().toJson(user, User.class,  file);
                file.close();
            } catch (IOException e) {
                throw new RuntimeException(e);
            }
            ae.getWindow().dispose();
    ...
    private void createMap(String path) {
        boolean loaded = mapManager.createMap(imageLoader, path, isResumeGame, user);
        if(loaded){
